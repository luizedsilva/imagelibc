...
void mediana(imagem In, imagem Out, int nl, int nc, int mn) {
    int i, j, vetor[9];
    for (i = 1; i < nl - 1; i++)
        for (j = 1; j < nc - 1; j++) {
...
void msg(char *s) {
    printf("\nFILTRO DA MEDIANA de uma imagem");
    printf("\n-------------------------------");
    printf("\nUSO.:  %s  <IMG.PGM>", s);
    printf("\nONDE:\n");
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2)
        ...
        if (OK) {
            printf("\nFILTRO DA MEDIANA DE IMAGEM");
            info_imagem(argv[1], nl, nc, mn);
            aloca_memo(&Out, nl, nc);
            mediana (In, Out, nl, nc, mn);
            sprintf(nome, "%s-semruido.pgm", argv[1]);
