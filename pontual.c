...
void transformacao(imagem In, imagem Out, int nl, int nc, int mn) {
    int i, j;
    float T[mn];
    for (i = 0; i < mn; i++)
        T[i] = (float)pow(i,.5)/pow(mn,.5) * mn;
    for (i = 0; i < nl; i++)
        for (j = 0; j < nc; j++) {
            Out[i * nc + j] = T[In[i*nc + j]];

            //Out[i * nc + j] = (float)pow(In[i * nc + j],.5)/pow(mn,.5) * mn;
            //Out[i * nc + j] = In[i * nc + j] > 100? 255 : 0; 
        }
}

void msg(char *s) {
    printf("\nTRANSFORMACAO de uma imagem qualquer");
    printf("\n-------------------------------");
    printf("\nUSO.:  %s  <IMG.PGM>", s);
    printf("\nONDE:\n");
