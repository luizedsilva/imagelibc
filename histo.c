/*+-------------------------------------------------------------------------+
  ...
  | -----------------------                                                 |
  | Programa que usa filtro mediana numa imagem em niveis de cinza.         |
  |                                                                         |
...
void equalizaHistograma (imagem In, imagem Out, int nl, int nc, int mn) {
    int i, j, histo[mn];
    float prob[mn];
    float sk[mn];
    int sk_arred[mn];
    ...
    for (i = 0; i < mn; i++)
       prob[i] = (float)histo[i] / (nl * nc);
    sk[0] = prob[0] * mn;
    sk_arred[0] = sk[0];
    for (i = 1; i < mn; i++) {
        sk[i] = sk[i-1] + prob[i] * mn;
        sk_arred[i] = sk[i];
    }
    for (i = 0; i < nl; i++)
        for (j = 0; j < nc; j++) {
            int px = In[i*nc + j];
            Out[i*nc + j] = sk_arred[px];
        }    
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2)
        ...
        if (OK) {
            ...
            equalizaHistograma (In, Out, nl, nc, mn);
            
            sprintf(nome, "%s-histograma.pgm", argv[1]);
